#!/usr/bin/env python3
"""fr30v6 ros2_control — Gazebo simulation with ros2_control"""
import os, tempfile
...
def generate_launch_description():
    share = pkg_dir("fr30v6_ros2_control")

    with open(os.path.join(share, "urdf", "fr30v6.urdf")) as f:
        desc = f.read()
    desc = desc.replace("package://fr30v6_ros2_control/", share + "/")
    ...
    tmp = tempfile.NamedTemporaryFile(mode='w', suffix='.urdf', delete=False, prefix='fr30v6_')
    tmp.write(desc); tmp.close()
    spawn = Node(package='gazebo_ros', executable='spawn_entity.py',
                 arguments=['-file', tmp.name, '-entity', 'fr30v6'], output='screen', additional_env={'GAZEBO_PLUGIN_PATH':'/opt/ros/humble/lib'})
